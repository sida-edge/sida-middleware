      ...
      connected_(false) 
{
    ...
    if (ctx_ == nullptr) {
        std::cerr << "[MODBUS] Failed to create Modbus context\n";
    }
}
...
bool ModbusDriver::connect() {
    ...
    connected_ = true;
    std::cout << "[MODBUS] Connected to " << config_.device_id << "\n";
...
void ModbusDriver::disconnect() {
    if (ctx_ && connected_) {
        modbus_close(ctx_);
        connected_ = false;
        std::cout << "[MODBUS] Disconnected from " << config_.device_id << "\n";
    }
}
...
std::vector<sida::TagRecord> ModbusDriver::pollData() {
    ...
    if (!connected_ || ctx_ == nullptr) {
        std::cerr << "[MODBUS] Device " << config_.device_id << " is not connected.\n";
        return data;
    }
    ...
    for (const auto& [metric_key, metric] : config_.metrics) {
        ...
        if (metric.register_type == "holding" || metric.register_type.empty()) {
            rc = modbus_read_registers(ctx_, metric.address, num_regs, raw_data.data()); 
        } else if (metric.register_type == "input") {
            rc = modbus_read_input_registers(ctx_, metric.address, num_regs, raw_data.data());
        } 
        else {
            std::cerr << "[MODBUS] Unsupported register type: " << metric.register_type << "\n";
            continue;
        }

        if (rc == -1) {
            std::cerr << "[MODBUS] Read failed for " << metric.name << ": " << modbus_strerror(errno) << "\n";
            break;
        }
        ...
        std::cout << "[MODBUS] Read " << metric.name << "\n";
